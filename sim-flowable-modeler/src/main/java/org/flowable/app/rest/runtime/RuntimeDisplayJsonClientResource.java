...
public class RuntimeDisplayJsonClientResource {
    ...
    public RuntimeDisplayJsonClientResource() {
        ...
        propertyMappers.put("UserTask", new MyUserTaskInfoMapper());
    }
    ...
    public JsonNode getModelJSON(@PathVariable String processInstanceId) {
        ...
        //changed started
        List<Task> taskList = taskService.createTaskQuery().processDefinitionId(processInstance.getProcessDefinitionId()).taskAssignee(currentUser.getId()).orderByTaskId().desc().list();
        String executionId = "";
        ...
        if (CollectionUtils.isNotEmpty(activityInstances)) {
            for (HistoricActivityInstance activityInstance : activityInstances) {
                ...
                } else if(!"".equals(executionId)?activityInstance.getExecutionId().equals(executionId):true){
            ...
            if (element instanceof SequenceFlow) {
                ...
                if(element instanceof UserTask) {
                    ...
                    for(HistoricActivityInstance historicActivityInstance : activityInstances){
                        if(historicActivityInstance.getActivityId().equals(element.getId())){
                            assignment = historicActivityInstance.getAssignee();
                            break;
                        }
                    }
                    if (propertyMappers.containsKey(className)) {
                        ((MyUserTaskInfoMapper)propertyMappers.get(className)).setCompleteOrCurrentTask(true);//assignment!=null
                        ((MyUserTaskInfoMapper)propertyMappers.get(className)).setAssignment(assignment);
                        elementNode.set("properties", propertyMappers.get(className).map(element));
                    }
                }else{
                    if (propertyMappers.containsKey(className)) {
